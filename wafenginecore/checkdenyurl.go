...
func (waf *WafEngine) CheckDenyURL(r *http.Request, weblogbean *innerbean.WebLog, formValue url.Values, hostTarget *wafenginmodel.HostSafe, globalHostTarget *wafenginmodel.HostSafe) detection.Result {
	...
	if globalHost != nil && globalHost.Host.GUARD_STATUS == 1 && globalHost.UrlBlockLists != nil {
		for i := 0; i < len(globalHost.UrlBlockLists); i++ {
			// 将全局规则URL也转为小写
			lowerGlobalRuleURL := strings.ToLower(globalHost.UrlBlockLists[i].Url)

			if (globalHost.UrlBlockLists[i].CompareType == "等于" && lowerGlobalRuleURL == lowerURL) ||
				(globalHost.UrlBlockLists[i].CompareType == "前缀匹配" && strings.HasPrefix(lowerURL, lowerGlobalRuleURL)) ||
				(globalHost.UrlBlockLists[i].CompareType == "后缀匹配" && strings.HasSuffix(lowerURL, lowerGlobalRuleURL)) ||
				(globalHost.UrlBlockLists[i].CompareType == "包含匹配" && strings.Contains(lowerURL, lowerGlobalRuleURL)) {
				weblogbean.RISK_LEVEL = 1
				result.IsBlock = true
