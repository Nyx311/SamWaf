...
chcp 65001
set currentpath=%cd%
set currentversion=v1.3.24
set currentdescription=请在闲时升级,升级前先查看详情，https://doc.samwaf.com/quickstart/Update.html
...
%currentpath%\setup\go_gen_updatefile\go_gen_updatefile.exe -desc %currentdescription% -o %currentpath%\release\web\samwaf_update -platform darwin-arm64 %currentpath%\release\githubci\%currentversion%\SamWafDarwinArm64 %currentversion%
%currentpath%\setup\go_gen_updatefile\go_gen_updatefile.exe -desc %currentdescription% -o %currentpath%\release\web\samwaf_update -platform darwin-amd64 %currentpath%\release\githubci\%currentversion%\SamWafDarwinAmd64 %currentversion%